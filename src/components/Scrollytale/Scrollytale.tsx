...
const Content = styled.div<IContentProps>`
    ...
    height: ${props=> props.cH}px;
`;


interface IScrollytale {
...
const Sticky2Styled = styled.div<IStep>`
  overflow: hidden;
  .section {
...
export const Scrollytale = (props: IScrollytale) => {

    // make sure a step is one screen
    const [height, setHeight] = React.useState(window && window.innerHeight ? window.innerHeight : 100);
    const [width, setWidth] = React.useState(window && window.innerWidth ? window.innerWidth : 100);

    return <Sticky2Styled screenHeight={height}>
        <Controller>
            <Scene
                triggerHook={/*for this is the first Step, it starts when scrolling*/ "onLeave"}
                duration={height}
                pin
                offset={-36}
            >
                {(progress) => {
                    ...
                    return <div className="sticky">
                        <Timeline totalProgress={Math.max(0,progress-0.7)} paused>
                            <Tween
                                from={{x: `${width-500}px`, top: `${200}px`}}
                                to={{x: `${width+100}px`, top: `${200}px`}}
                            >
                                <Area/>
                            </Tween>
                            <Timeline
